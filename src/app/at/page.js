  ...
  return (
    ...
    >
      <div className="p-4 flex flex-col items-center">
        <div className="w-24 h-8 bg-white rounded-full flex items-center justify-center mb-4">
          <span className="text-red-600 font-bold">AirtelTigo</span>
        </div>
        <div className="text-3xl font-bold text-white mb-4">{bundle.capacity}GB</div>
        <div className="w-full grid grid-cols-2 bg-blue-900 text-white">
          <div className="p-4 text-center border-r border-blue-800">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="group">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1 group-hover:text-red-600 transition-colors">
                Recipient Number
                <span className="text-xs text-gray-500 dark:text-gray-400 ml-1">(e.g., 0271234567)</span>
              </label>
...
const ATOrderPage = () => {
  ...
  const fetchBundles = async () => {
    try {
      ...
      // Find AT_PREMIUM network configuration
      const atNetwork = networks.find(n => n.networkKey === 'AT_PREMIUM');
      
      if (!atNetwork) {
        console.error('AT_PREMIUM network not found. Available networks:', networks.map(n => n.networkKey));
        setError('AirtelTigo network not available. Please contact support.');
        return;
      }
      
      console.log(`Found AirtelTigo network (active: ${atNetwork.isActive}) with ${atNetwork.bundles.length} bundles`);
      
      if (!atNetwork.bundles || atNetwork.bundles.length === 0) {
        setError('No bundles configured for AirtelTigo network');
        return;
      }
      
      // Sort bundles by capacity
      const sortedBundles = atNetwork.bundles.sort((a, b) => parseFloat(a.capacity) - parseFloat(b.capacity));
      setBundles(sortedBundles);
      
      // Check if network is inactive
      if (!atNetwork.isActive) {
        console.log('Warning: AirtelTigo network is currently inactive. All bundles will show as out of stock.');
      }
      
      const activeBundles = sortedBundles.filter(b => b.isActive);
      console.log(`AirtelTigo bundles loaded: ${sortedBundles.length} total, ${activeBundles.length} available`);
  ...
  if (!userAuthenticated) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-red-50 via-white to-blue-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 p-4 flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-red-50 via-white to-blue-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 p-4">
      {/* Header with logout button */}
      <div className="max-w-6xl mx-auto mb-6 flex justify-between items-center">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">AirtelTigo Data Bundles</h1>
        <button
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-orange-800 dark:text-orange-300">
                {bundles[0]?.networkInactive 
                  ? 'AirtelTigo network is currently unavailable. All bundles are temporarily out of stock.'
